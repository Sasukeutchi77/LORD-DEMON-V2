...
export async function handleMessage(sock, msg, stats, afkStore, opts = {}) {
  ...
  // ══════════════════════════════════════════════════════════
  // DÉTECTION DES RÉPONSES INTERACTIVES (boutons / listes)
  // ──────────────────────────────────────────────────────────
  // Quand l'utilisateur appuie sur un bouton du menu interactif
  // (.menubtn), WhatsApp envoie un message spécial contenant l'ID
  // de la ligne/bouton sélectionné. On le traite comme si l'utilisateur
  // avait tapé la commande correspondante.
  //
  // Types gérés :
  //  • listResponseMessage      → tap sur une ligne de list message
  //  • buttonsResponseMessage   → tap sur un bouton classique
  //  • templateButtonReplyMessage → tap sur un bouton template
  // ══════════════════════════════════════════════════════════
  const _interactiveReply =
  ...
  // ── ANTI-LINK ─────────────────────────────────────────────
  // On saute l'anti-link pour les réponses interactives (rowId ne contient jamais de lien)
  if (isGroup && bodyRaw && !_interactiveReply) {
  ...
  const ctx = {
    ...
    isInteractiveReply: !!_interactiveReply,
  }
  ...
  try {
    ...
    const botLabel = opts.isSecondaryBot
      ...
      : '[bot principal]'
    const source = _interactiveReply ? '🔘 bouton' : (isGroup ? 'groupe' : 'privé')
    console.log(`⚡ ${cmdName} | ${cleanNumber(senderJid)} | ${source} ${botLabel}`)
