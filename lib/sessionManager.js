...
export async function createBotInstance(number, onPairingCode, isReconnection = false) {
  ...
  // ── Pairing code : déclenché par l'event "qr" de Baileys ────────────
  // requestPairingCode doit être appelé quand le serveur signale qu'il attend
  // une authentification (event qr). Tout autre timing génère un code invalide.
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    try {
      // ── Pairing code : déclenché par l'event qr (signal WS prêt) ──────
      if (qr && onPairingCode && !pairingDone && !sock.authState.creds.registered) {
        pairingDone = true
        try {
          ...
          pairingDone = false  // autoriser retry si qr retentit
        }
      }
