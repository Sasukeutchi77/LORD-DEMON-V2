// commands/quiz-musique2.js
import { sendMessage } from '../lib/sendMessage.js'

const QUESTIONS = [{"q":"Qui a chante Thriller ?","r":"Michael Jackson","opts":["Prince","Michael Jackson","David Bowie","Freddie Mercury"]},{"q":"De quel pays vient le groupe ABBA ?","r":"Suede","opts":["Norvege","Danemark","Suede","Finlande"]},{"q":"Quelle artiste a sorti l album Lemonade ?","r":"Beyonce","opts":["Rihanna","Beyonce","Adele","Lady Gaga"]},{"q":"Combien de membres compte le groupe BTS ?","r":"7","opts":["5","6","7","8"]}]
const sessions = new Map()

export default async function cmd_quiz_musique2(sock, sender, args, msg) {
  const s = sessions.get(sender)
  if (s && args.length) {
    const ans = args.join(' ').trim()
    sessions.delete(sender)
    const correct = ans.toLowerCase() === s.r.toLowerCase() || s.opts.some(o => o.toLowerCase() === ans.toLowerCase())
    return sendMessage(sock, sender, correct ? '✅ CORRECT ! Reponse: *' + s.r + '*' : '❌ FAUX ! C etait: *' + s.r + '*')
  }
  const q = QUESTIONS[Math.floor(Math.random() * QUESTIONS.length)]
  sessions.set(sender, q)
  setTimeout(() => sessions.delete(sender), 30000)
  const opts = q.opts.map((o, i) => ['A','B','C','D'][i] + '. ' + o).join('\n')
  await sendMessage(sock, sender, 'X┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈X\n⛧   QUIZ MUSIQUE 2   ☩\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n❓ ' + q.q + '\n\n' + opts + '\n\n⏱️ 30 sec ! Tapez la reponse.\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
}
