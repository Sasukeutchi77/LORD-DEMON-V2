...
export default async function bite2(sock, sender, args, msg, ctx) {
  ...
  if (!target) return await sendMessage(sock, sender, `☠ Usage: ${prefix}bite2 @user`)
  const from = senderJid.replace('@s.whatsapp.net','')
  const to = target.replace('@s.whatsapp.net','')
  await sock.sendMessage(sender, { text:`☩━━━〔 😬 *BITE2* 〕━━━☩\n☠\n⛧  @${from} mord @${to}\n☠\n⸸━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━⸸`, mentions:[senderJid,target] }).catch(()=>sendMessage(sock,sender,'😬 mord!'))
}